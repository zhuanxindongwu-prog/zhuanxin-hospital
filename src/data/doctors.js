export const doctors = [
  {
    id: 'hung-rong-wei',
    name: '洪榮偉 院長',
    role: 'Director / Veterinary Cardiologist',
    title: '獸醫心臟專科醫師',
    image: '/imgs/DRH.webp',
    award: "FASAVA-Hill's Practitioner of the Year Award 2023",
    awardZh: '亞洲年度卓越獸醫師獎',
    credentials: [
      '國立台灣大學獸醫學士',
      '美國俄亥俄州立大學獸醫學碩士',
      '美國北卡羅來納州立大學心臟科訓練',
      '亞洲獸醫內科專科醫學院心臟次專科認證醫師',
      '亞洲獸醫內科專科醫學院心臟次專科主席',
      'Fellow of American Society of Echocardiography'
    ],
    specialties: ['犬貓心臟病', '心臟超音波', '心衰竭管理', '心律不整'],
    intro:
      '洪榮偉院長為國際知名之小動物心臟疾病權威，具備豐富的臨床經驗與專業知識。多年來致力於犬貓心臟疾病診療、研究與教育，並培育出許多台灣小動物心臟主治醫師。',
    aboutTitle: '關於洪院長',
    about: [
      '洪院長長年在第一線服務犬貓心臟病患，深受寵物家長與獸醫師信賴。除了臨床診療外，也曾於多個國際獸醫學會發表演講與研究論文，持續推動小動物心臟醫學的發展。',
      '洪院長視病如親，總是以親切且專業的態度關照每一位專心寶貝與家長，是心臟病患長期治療過程中最堅強且溫暖的後盾。'
    ],
    tags: ['心臟超音波', '心衰竭管理', '心律不整']
  },
  {
    id: 'lin-zheng',
    name: '林正 副院長',
    role: 'Veterinary Internal Medicine',
    title: '獸醫內科醫師',
    image: '/imgs/DRL.webp',
    award: '',
    awardZh: '',
    intro: '林正副院長專注於犬貓內科疾病與慢性病照護，協助病患進行完整評估與長期追蹤。',
    aboutTitle: '關於林副院長',
    about: [
      '林副院長重視完整病史、理學檢查與影像判讀，協助飼主理解疾病變化與治療方向。',
      '在慢性病與心臟相關照護中，提供細緻且穩定的醫療陪伴。'
    ],
    tags: ['心臟科', '慢性病', 'ECG判讀', '麻醉術前評估']
  },
  {
    id: 'lee-ji-zhong',
    name: '李繼忠 醫師',
    role: 'Veterinary Medical Oncologist',
    title: '獸醫腫瘤內科醫師',
    image: '/imgs/DRLEE.webp',
    award: '',
    awardZh: '',
    intro: '李繼忠醫師專注於犬貓腫瘤內科診療，協助病患進行腫瘤評估、治療規劃與生活品質照護。',
    aboutTitle: '關於李醫師',
    about: [
      '李醫師重視個別化醫療，依照病患狀況、疾病分期與家庭需求制定合適治療策略。',
      '致力於在治療效果與生活品質之間取得平衡。'
    ],
    tags: ['腫瘤內科', '化療評估', '個別化治療']
  },
  {
    id: 'wu-guan-xun',
    name: '吳冠勳 醫師',
    role: 'Veterinary Cardiologist',
    title: '獸醫心臟專科醫師',
    image: '/imgs/all.webp',
    award: '',
    awardZh: '',
    intro: '吳冠勳醫師專注於犬貓心臟疾病診療與追蹤照護。',
    aboutTitle: '關於吳醫師',
    about: [
      '提供心臟超音波、心律評估與心臟病長期治療建議。',
      '協助飼主理解疾病分期與日常照護重點。'
    ],
    tags: ['心臟超音波', '心臟病治療', '長期追蹤']
  },
  {
    id: 'yan-shi-rong',
    name: '顏士容 醫師',
    role: 'Veterinary Cardiologist',
    title: '獸醫心臟專科醫師',
    image: '/imgs/all.webp',
    award: '',
    awardZh: '',
    intro: '顏士容醫師專注於犬貓心臟疾病評估與治療。',
    aboutTitle: '關於顏醫師',
    about: [
      '以細緻的檢查與溝通協助飼主了解毛孩的心臟狀況。',
      '重視長期追蹤與個別化治療。'
    ],
    tags: ['心臟科', '心衰竭管理', 'ECG判讀']
  },
  {
    id: 'chen-shi-ting',
    name: '陳詩婷 醫師',
    role: 'Veterinary Cardiologist',
    title: '獸醫心臟專科醫師',
    image: '/imgs/all.webp',
    award: '',
    awardZh: '',
    intro: '陳詩婷醫師專注於犬貓心臟疾病診療。',
    aboutTitle: '關於陳醫師',
    about: [
      '協助病患進行心臟檢查、治療規劃與穩定追蹤。',
      '重視飼主溝通與居家照護教育。'
    ],
    tags: ['心臟科', '慢性病', '麻醉術前評估']
  },
  {
    id: 'chen-xuan-zhen',
    name: '陳炫甄 醫師',
    role: 'Veterinary Cardiologist',
    title: '獸醫心臟專科醫師',
    image: '/imgs/all.webp',
    award: '',
    awardZh: '',
    intro: '陳炫甄醫師專注於犬貓心臟疾病診療與影像評估。',
    aboutTitle: '關於陳醫師',
    about: [
      '提供心臟相關檢查與個別化治療建議。',
      '協助飼主掌握疾病變化與照護方向。'
    ],
    tags: ['心臟超音波', '心臟科', 'ECG判讀']
  },
  {
  id: 'chen-pin-qi',
  name: '陳品齊 醫師',

  role: 'Resident Veterinarian',
  title: '住院獸醫師',

  image: '/imgs/chenpingqi.jpg',

  intro:
    '專注於犬貓心血管疾病診療與住院照護，致力於提供細緻且穩定的臨床醫療品質，並持續精進小動物內科與心臟醫學相關知識。',

  aboutTitle: '關於陳品齊醫師',

  about: [
    '目前於專心動物醫院接受住院醫師訓練。',
    '參與犬貓心臟疾病、重症照護與住院管理。',
    '對心臟超音波與臨床影像評估具有高度興趣。',
    '重視與飼主之間的溝通，希望提供清楚且溫暖的醫療照護。'
  ],

  specialties: [
    '犬貓內科',
    '住院與重症照護',
    '心臟疾病評估',
    '心臟超音波學習'
  ]
}
]
